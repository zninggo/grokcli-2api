...
function rebindPageControls() {
  ...
  try { initJsonExportModeControls(); } catch (_) {}
...
function getJsonExportMode() {
  const el = $("export-mode") || $("export-mode-json");
  const raw = ((el && el.value) || "access_only").trim().toLowerCase();
  if (raw === "full" || raw === "metadata" || raw === "access_only") return raw;
  return "access_only";
}

function syncJsonExportModeSelects(mode) {
  const m = mode || getJsonExportMode();
  ["export-mode", "export-mode-json"].forEach((id) => {
    const el = $(id);
    if (el && el.value !== m) el.value = m;
  });
  try { localStorage.setItem("g2a_export_mode", m); } catch (_) {}
  return m;
}

function initJsonExportModeControls() {
  let saved = "access_only";
  try {
    const v = localStorage.getItem("g2a_export_mode");
    if (v === "full" || v === "metadata" || v === "access_only") saved = v;
  } catch (_) {}
  ["export-mode", "export-mode-json"].forEach((id) => {
    const el = $(id);
    if (!el) return;
    el.value = saved;
    el.onchange = () => syncJsonExportModeSelects(el.value);
  });
}
...
async function runJsonExportJob({ mode = "all", ids = [], buttonId = "btn-export" } = {}) {
  ...
  const exportMode = syncJsonExportModeSelects(getJsonExportMode());
  const modeLabel = {
    full: "完整备份",
  ...
  if (exportMode === "full") {
    const ok = confirm(
      "将导出「完整备份」，包含 refresh_token / SSO / 密码。\n" +
      "他人拿到后可长期续期，还可能互相刷新导致 401。\n\n" +
      "仅在自用迁移/备份时继续；对外分享请改选「仅 access」。"
    );
    if (!ok) return;
  }
  if (btn) {
  ...
  setJsonIoProgress({
    percent: 0,
    label: mode === "selected"
      ? `开始导出选中 ${selectedN} 个账号（${modeLabel}）…`
      : `开始导出全部账号（${modeLabel}）…`,
    detail: "提交任务中",
    done: 0,
  ...
  setLogPanel("json-io-result", `提交导出任务… mode=${exportMode}`, { forceShow: true });
  try {
    let started;
    if (mode === "selected") {
      started = await api("/accounts/export-batch?async_job=1", {
        method: "POST",
        body: JSON.stringify({ ids, export_mode: exportMode }),
      });
    } else {
      const qs = new URLSearchParams({ async_job: "1", export_mode: exportMode });
      started = await api(`/accounts/export?${qs.toString()}`);
    }
    ...
    if (started && started.auth && !started.job_id) {
      ...
      const suffix = exportMode === "full" ? "full" : exportMode === "metadata" ? "metadata" : "access-only";
      a.download = mode === "selected"
        ? `grok2api-auth-export-selected-${suffix}-${selectedN}.json`
        : `grok2api-auth-export-${suffix}.json`;
      ...
      toast(`已导出 ${started.count || selectedN || ""} 个账号（${modeLabel}）`);
      return;
    }
    ...
    toast(finalJob.message || `已导出 ${finalJob.count || selectedN || ""} 个账号（${modeLabel}）`);
  } catch (e) {
    setJsonIoProgress({
      percent: 100,
      label: "导出失败",
